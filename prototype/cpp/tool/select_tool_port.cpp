...
/*
 * select_tool_port.cpp — port of Inkscape ui/tools/select-tool.cpp (alpha).
 *
 * Interaction logic copied VERBATIM from the Inkscape sources (see the
 * header for provenance). Adaptations, each marked ADAPTACIÓN:
 *   - includes redirected to the sham/ harness (rect-tool-port recipe);
 *   - GTK window/cursor glue trimmed (no window, no mouse cursor in alpha);
 *   - `delta.y()` (2geom accessor) -> `delta[Geom::Y]` (sham Point fields).
 * The SelTrans behind `_seltrans` is the alpha engine (sham/seltrans) which
 * exposes the same grab/moveTo/ungrab/increaseState API.
 */
#include "select_tool_port.h"

#include "undo.h"                 // original: document-undo.h
#include "desktop.h"              // original: document.h (SPDocument here)
#include "layer_manager.h"        // original: layer-manager.h
#include "selection_chemistry.h"  // original: selection-chemistry.h
#include "selection_describer.h"  // original: selection-describer.h
#include "selection.h"            // original: selection.h
#include "seltrans.h"             // original: seltrans.h
#include "actions_tools.h"        // original: actions/actions-tools.h (set_active_tool)
#include "prefs.h"                // original: Preferences (via tool_base.h too)
#include "sp_rect.h"              // original: object/box3d.h + display/drawing-item.h
#include "canvas_item.h"          // original: display/control/canvas-item-*.h + ui/widget/canvas.h
#include "snap.h"                 // sp_event_context_find_item / sp_event_context_read

using Inkscape::DocumentUndo;
using Inkscape::Modifiers::Modifier;
using Inkscape::UI::Events::EventType;
...
void SelectTool::sp_select_context_cycle_through_items(Selection *selection, ScrollEvent const &scroll_event)
{
    ...
    // ADAPTACIÓN (alpha): original `scroll_event.delta.y()` ("<0" scroll up,
    // ">0" scroll down); the sham Point has plain x/y fields, so the accessor
    // is written as axis indexing (same value).
    if (scroll_event.delta[Geom::Y] < 0) {
        if (! cycling_cur_item) {
    ...
    } else if (scroll_event.delta[Geom::Y] > 0) {
        if (! cycling_cur_item) {
...
bool SelectTool::root_handler(CanvasEvent const &event)
{
    ...
    inspect_event(event,
        ...
        [&] (ScrollEvent const &event) {
            ...
            // ADAPTACIÓN (alpha): the original brings the GtkWindow to the
            // front here (gtk_widget_get_root / gtk_window_present); the alpha
            // canvas is the whole screen, so only grab_focus() survives.
