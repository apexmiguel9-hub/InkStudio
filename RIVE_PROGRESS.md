...
| CI run #8 (falla ~5 min): rive OK; nuestro código avanza — tool/sham+renderer.cpp compilan limpios; mueren los TU del bootstrap con `ANDROID_LOG_ERROR` undeclared | ~15 min | our tool/sham y renderer.cpp FIRST CLEAN compile. `logging.hpp` incluye `<android/log.h>` solo bajo `#ifdef RIVE_ANDROID` pero usa `ANDROID_LOG_ERROR` bajo `__ANDROID__`; premake define `RIVE_ANDROID` vía `filter('system:android')` (premake5_pls_renderer.lua:101) y nuestro CMake no lo replicaba | `RIVE_ANDROID` en target_compile_definitions (espejo del filter premake) |
| CI run #9 (falla ~6 min): **TODO compila (rive, nuestro C++ completo, bootstrap) — muere en el LINK** con `undefined symbol: rive::DecodeKtx2 / Bitmap::decode / Bitmap::pixelFormat` | ~20 min | las 3 refs viven SOLO en render_context.cpp bajo `#ifdef RIVE_KTX2` (DecodeKtx2) y `#ifdef RIVE_DECODERS` (Bitmap) — premake los define por defecto → pls renderer referencia la lib de decoders (png/jpeg/webp/zlib), que no compilamos | seed de args con `--no-rive-decoders`: los guards se compilan fuera (escena de paths, jamás decodificamos imágenes; platformDecodeImageTexture del impl vulkan devuelve nullptr; core no referencia Bitmap salvo wasm/lua) |
| CI run #10 (falla ~2 min): `error: premake5 arguments for current build do not match previous arguments` | ~10 min | arreglo del #9 aplicado al seed del archivo, pero el env `RIVE_PREMAKE_ARGS` seguía sin `--no-rive-decoders` → el check de build_rive.sh (línea ~364) falló antes de premake (ver bug #7) | env con `--with_vulkan --no-rive-decoders`; MATCH byte a byte verificado |
...
| 5 | `vulkan_debug_callbacks.cpp:104: use of undeclared identifier 'ANDROID_LOG_ERROR'` | `renderer/rive_vk_bootstrap/src/logging.hpp` incluye `<android/log.h>` solo bajo `#ifdef RIVE_ANDROID`, pero expande `LOG_ERROR_LINE` (que usa `ANDROID_LOG_ERROR`) bajo `#if defined(__ANDROID__)`. premake define `RIVE_ANDROID` vía `filter('system:android')` (premake5_pls_renderer.lua:101); nuestro CMake no lo replicaba | añadir `RIVE_ANDROID` a target_compile_definitions (espejo del filter) |
| 6 | link: `undefined symbol: rive::DecodeKtx2 / rive::Bitmap::decode / Bitmap::pixelFormat` (ld.lld) | premake define `RIVE_DECODERS`+`RIVE_KTX2` por defecto (filters `options:not no-rive-decoders` en premake5_pls_renderer.lua:595-603); activos, render_context.cpp referencia la lib `rive_decoders` (png/jpeg/webp/zlib) que no compilamos ni enlazamos | `--no-rive-decoders` en el seed de args premake: guards `#ifdef RIVE_DECODERS/RIVE_KTX2` se compilan fuera; el core solo referencia `Bitmap::decode` en wasm/lua (no compilados); nuestro uso es solo paths |
| 7 | `error: premake5 arguments for current build do not match previous arguments` (exit 255, 2 min) | build_rive.sh línea ~364 compara el string **ensamblado del CLI** contra `.rive_premake_args`; el env `RIVE_PREMAKE_ARGS` que pasábamos solo llevaba `--with_vulkan`, así que el "current" salía sin `--no-rive-decoders` → mismatch con el seed nuevo | `RIVE_PREMAKE_ARGS="--with_vulkan --no-rive-decoders"` en la invocación de build_rive.sh (env = también lo que premake recibe; verificado MATCH byte a byte con la lógica de ensamblado línea 237) |
